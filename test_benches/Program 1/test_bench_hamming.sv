...
// mem[16,17] = min & max Hamming distances among sdata pairs
    ...
    $readmemb("test1_2.txt",D1.data_mem.core);
    //$readmemb("d_machine_p1.txt",D1.Instr_mem.core);

    for(int i=0; i<16; i++) begin
      Tmp[i] = {D1.data_mem.core[i]};
      $display("%d:  %b",i,Tmp[i]);
	end
	$display();                              // line-space
// DUT data memory preloads beyond [15] (next 3 lines of code)
    D1.data_mem.core[16] = 'd8;		             // preset DUT final Min 
...
// 	compute correct answers
	...
	//D1.data_mem.core[16] = Min;
	//D1.data_mem.core[17] = Max;
	#200ns reset = 'b0;
	#200ns start = 'b0; 
    #200ns wait (done);						 // avoid false done signals on startup
								 
// check results in data_mem[64] and [65] (Minimum and Maximum distances, respectively)
    if(Min == D1.data_mem.core[16]) $display("good Min = %d",Min);
	else                      $display("fail Min: Correct = %d; Yours = %d",Min,D1.data_mem.core[16]);
                              $display("Min addr = %d, %d",Min1, Min2);
							  $display("Min valu = %b, %b",Tmp[Min1],Tmp[Min2]);//{D1.dm.core[2*Min1],D1.dm.core[2*Min1+1]},{D1.dm.core[2*Min2],D1.dm.core[2*Min2+1]});
	if(Max == D1.data_mem.core[17]) $display("good Max = %d",Max);
	else                      $display("MAD  Max: Correct = %d; Yours = %d",Max,D1.data_mem.core[17]);
